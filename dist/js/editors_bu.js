...
var tc_class = {
    ...
    createSQLSave: function (sender, rec) {
        ...
        for (var i = 0; i < savefieldlist.length; i++) {
            ...
            if (app.isP)
                ...
                val = '@' + savefieldlist[i] + ' = ' + val;

            vals.push(val);
        }
        var sql = '';
        if (app.isP)
            sql = 'select * from ' + sender.finder.t_rpdeclare.editproc + '(' + vals.join(',') + ')';
        else
            sql = 'exec ' + sender.finder.t_rpdeclare.editproc + ' ' + vals.join(',');
        //console.debug(sql);
        return sql;
                ...
                if (tp == 'text') {
                    var jr = app.t_sysfieldmap.filter(function (value) {
                        return (
                            ...
                            (
                                (value['dstfield'].toLowerCase() == sender.fields[i].field && app.isP) ||
                                (value['dstfield'] == sender.fields[i].field && !app.isP)
                            ) &&
                    ...
                    if (jr.length == 1) {
                        ...
                        for (var j = 0; j < jrf.length; j++) {
                            if (app.isP)
                                ...
                                sender.fields[i].joinRow.fields[jrf[j]['dstfield']] = jrf[j]['srcfield'];
                        }
                        ...
                        if (sender.fields[i].joinRow.classname == 'Bureau.GridCombo') {
                            ...
                            if (app.isP)
                                ...
                                sender.fields[i].joinRow.textField = jr[0]['srcfield'];
                        }
                    }
                }
        ...
        for (var i = 0; i < sender.fields.length; i++) {
            ...
            if (tp == 'date')
                ...
                if (tp == 'datetime')
                    ...
                    if (tp == 'numeric')
                        $(sender.id(fname)).numberbox({
                            precision: sender.fields[i].precision,
                            decimalSeparator: ',',
                            onChange: function (newValue, oldValue) {
    ...
    addrecord: function (sender) {
        ...
        var keyfield = "";
        if (app.isP)
            keyfield = sender.finder.t_rpdeclare.keyfield.toLowerCase();
        else
            keyfield = sender.finder.t_rpdeclare.keyfield;
        ...
        $.messager.confirm(
            {
                ...
                fn: function (r) {
                    if (r) {
                        ...
                        if (!app.isP)
                            val = "'" + row[sender.finder.t_rpdeclare.keyfield].toString().replace(/'/g, "''") + "'";
                        else
...
var column_editor = {
    ...
    show: function (sender) {
        ...
        if (!app.isP)
            sql = "select paramvalue from t_sysParams  where  paramname = 'GridFind" + sender.editform.record['DecName'] + "'";
        $.post('/pg/runsql', {
            sql: sql//,
            //account: app.account,
            //password: app.password
    ...
    initdata(sender, sXML) {
        ...
        if (!app.isP)
            sql = sender.editform.record['DecSQL'];
        $.post('/pg/runsql', {
            sql: sql//,
            //account: app.account,
            //password: app.password
    ...
    SaveXML: function (sender) {
        ...
        if (!app.isP)
            ParamName = 'GridFind' + sender.editform.record['DecName'];
        var ParamValue = $('#teXML').val();
        var sql = "select p_lbrsetparam ('" + ParamName + "', '" + ParamValue + "', '" + ParamName + "')";
        if (!app.isP)
            sql = "exec p_lbrsetparam '" + ParamName + "', '" + ParamValue + "', '" + ParamName + "'";
