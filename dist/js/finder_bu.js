            ...
            if (tab)
...

                {
                    ...
                    if (node.attributes.link1.indexOf('/') > -1) {
                        ...
                        if (node.attributes.link1 == "Bureau.Access") {
    ...
    addAcc: function (sender) {
        ...
        if (row) {
            ...
            if (freerow) {
                ...
                if (!app.isP)
                    sql = "exec p_sysAccesUser '" + row["ObjectName"] + "', '" + freerow["Account"] + "', 1";
                $.post('/pg/runsql',
                    {
                        sql: sql//,
                        //account: app.account,
    ...
    deleteAcc: function (sender) {
        ...
        if (row) {
            ...
            if (freerow) {
                ...
                if (!app.isP)
                    sql = "exec p_sysAccesUser '" + row["ObjectName"] + "', '" + freerow["ACCOUNT"] + "', 2";
        ...
        if (sender.t_rpdeclare['keyvalue'] > 0) {
            ...
            sender.lbut.linkbutton({
                ...
                onClick: function () {
                    ...
                    if (app.isP)
                    {
                        node_text = sender.t_rpdeclare.descr + "_" + row[sender.t_rpdeclare.dispfield.toLowerCase()].toString();
                        node_id = sender.t_rpdeclare.descr + "_" + row[sender.t_rpdeclare.keyfield.toLowerCase()].toString();
                    }
                    else
                    {
                        node_text = sender.t_rpdeclare.descr + "_" + row[sender.t_rpdeclare.dispfield].toString();
                        node_id = sender.t_rpdeclare.descr + "_" + row[sender.t_rpdeclare.keyfield].toString();
                    }
                    ...
                    if (tab)
                        ...
                        {
                            form1.SQLParams[sender.t_rpdeclare.keyfield.toLowerCase()] = row[sender.t_rpdeclare.keyfield.toLowerCase()].toString();
                            form1.DefaultValues[sender.t_rpdeclare.keyfield.toLowerCase()] = row[sender.t_rpdeclare.keyfield.toLowerCase()].toString();
                        }
